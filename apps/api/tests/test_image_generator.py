...
def test_story_content_kind_composites_overlay_text_onto_image() -> None:
    # CIN-123: Instagram Stories have no text-overlay parameter via the
    # API, so a story's generated text is rendered onto the image
    # itself before upload -- distinguish the two generateContent calls
    # (caption vs. overlay text) by a marker unique to each prompt.
    def handler(request: httpx.Request) -> httpx.Response:
        if "interactions" in str(request.url):
            return httpx.Response(
                200,
                json={
                    "status": "completed",
                    "output_image": {"data": "ZmFrZS1pbWFnZQ==", "mime_type": "image/jpeg"},
                },
            )
        body = json.loads(request.content)
        prompt = body["contents"][0]["parts"][0]["text"]
        if "поверх фото" in prompt:
            text = "Свежий кофе"
        else:
            text = "Полная подпись про кофе"
        return httpx.Response(200, json={"candidates": [{"content": {"parts": [{"text": text}]}}]})

    payload = {"topic": "утренний кофе", "platform": "instagram", "content_kind": "story"}
    with (
        patch(
            "app.content_pipeline.image_generator.composite_story_text",
            return_value=b"composited-bytes",
        ) as composite,
        patch(
            "app.content_pipeline.image_generator.upload_bytes",
            return_value="https://media.cindra.example/story.png",
        ) as upload,
    ):
        result = nano_banana_image_generator(payload, client=_client(handler))

    composite.assert_called_once_with(b"fake-image", "Свежий кофе")
    upload.assert_called_once_with(b"composited-bytes", "image/png", "png")
    assert result["image_url"] == "https://media.cindra.example/story.png"
    assert result["text"] == "Полная подпись про кофе"  # Post.text stays the full caption


def test_non_story_content_kind_does_not_composite_overlay_text() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if "interactions" in str(request.url):
            return httpx.Response(
                200,
        ...
        return httpx.Response(200, json={"candidates": [{"content": {"parts": [{"text": "Подпись"}]}}]})

    payload = {"topic": "утренний кофе", "platform": "instagram", "content_kind": "post"}
    with (
        patch("app.content_pipeline.image_generator.composite_story_text") as composite,
        patch(
            "app.content_pipeline.image_generator.upload_bytes",
            return_value="https://media.cindra.example/abc.png",
        ) as upload,
...
def test_story_overlay_text_failure_still_returns_plain_image() -> None:
    # Overlay text is best-effort (like the caption) -- a failure must
    # not fail the already-successful, already-paid-for image.
    def handler(request: httpx.Request) -> httpx.Response:
        if "interactions" in str(request.url):
            return httpx.Response(
                200,
    ...
    payload = {"topic": "x", "platform": "instagram", "content_kind": "story"}
    with (
        patch("app.content_pipeline.image_generator.composite_story_text") as composite,
        patch(
            "app.content_pipeline.image_generator.upload_bytes",
